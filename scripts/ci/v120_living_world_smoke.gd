...
func _run() -> void:
    ...
    if failures.is_empty():
        print("[V1.20 LIVING WORLD] Hearth growth, direction-aware Wanderer, readable harvesting and silhouettes passed")
    _finish()
...
func _fail(message: String) -> void:
    ...
    print("[V1.20 LIVING WORLD] FAIL: ", message)
...
func _finish() -> void:
    ...
    for failure: String in failures:
        push_error("[V1.20 LIVING WORLD] %s" % failure)
    get_tree().quit(1)
