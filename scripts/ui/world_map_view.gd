...
func _refresh_buttons() -> void:
    ...
    for i: int in range(node_buttons.size()):
        ...
        if i == 3:
            var frontier_open: bool = GameState.frontier_signal_unlocked()
...
func _layout_buttons() -> void:
    ...
    node_points = [
        Vector2(w * 0.34, h * 0.74),
        Vector2(w * 0.68, h * 0.50),
        Vector2(w * 0.37, h * 0.28),
        Vector2(w * 0.68, h * 0.10)
    ]
    for i: int in range(mini(node_buttons.size(), node_points.size())):
        ...
        if i == 0:
            ...
            button.position = p + Vector2(-176, -24)
        button.size = Vector2(146, 58)

func _on_node_pressed(index: int) -> void:
    if index == 3:
        if GameState.frontier_signal_unlocked():
...
func _draw() -> void:
    ...
    var frontier_color: Color = Color(0.64, 0.67, 0.90, 0.40) if GameState.frontier_signal_unlocked() else Color(0.38, 0.40, 0.43, 0.20)
    draw_line(node_points[2], node_points[3], frontier_color, 2.0)
...
func _draw_frontier_node(pos: Vector2) -> void:
    var unlocked: bool = GameState.frontier_signal_unlocked()
