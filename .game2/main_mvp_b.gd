...
func _draw_hud() -> void:
	draw_rect(Rect2(0, 0, VIEW_SIZE.x, 82), Color(0.04, 0.06, 0.045, 0.78))
	draw_string(font, Vector2(18, 28), "AXEHOLD", HORIZONTAL_ALIGNMENT_LEFT, 116, 20, Color("#f2e6ce"))
	draw_string(font, Vector2(137, 27), "BUILD B · ДОБЫЧА", HORIZONTAL_ALIGNMENT_LEFT, 230, 13, Color("#b8c5af"))

	var load := carried_wood + carried_stone
	draw_string(font, Vector2(18, 57), "ДЕРЕВО %d   КАМЕНЬ %d   ГРУЗ %d/%d" % [carried_wood, carried_stone, load, CARRY_LIMIT], HORIZONTAL_ALIGNMENT_LEFT, 390, 12, Color("#ddd1b8"))
	...
	if not joystick_active and not paused:
		var hint := "Подойди к дереву или камню — добыча начнётся сама"
		if load >= CARRY_LIMIT:
			hint = "Груз заполнен"
