...
const CLEAN_IDLE_BACK_PATH := "res://assets/art/visual_v2/hero_frames/back_a.webp"
...
func _draw() -> void:
    ...
    if absf(dir.x) > absf(dir.y) * 0.78:
        _draw_clean_texture(clean_side, Vector2(0, bob), lean, signf(dir.x), 90.0, 1.00)
    elif dir.y < -0.12:
        # Up-facing now uses the same illustrated hero family as front/side.
        # A slight width normalization keeps the silhouette consistent without
        # bringing back the old high-resolution wanderer asset.
        _draw_clean_texture(clean_back, Vector2(0, bob - 1.0), lean * 0.45, 1.0, 90.0, 1.10)
    else:
        _draw_clean_texture(clean_front, Vector2(0, bob), -lean * 0.35, 1.0, 90.0, 1.00)
...
func _draw_clean_texture(texture: Texture2D, offset: Vector2, rot: float, mirror: float, target_height: float, width_scale: float = 1.0) -> void:
    if texture == null:
        return
    ...
    var draw_size := Vector2(tex_size.x * scale_factor * width_scale, target_height)
...
func _draw_orbit_weapons(moving: bool) -> void:
    ...
    for i: int in range(count):
        ...
        var tex_size: Vector2 = texture.get_size()
        var target_h: float = 43.0 if weapon_style != "hammer" else 47.0
